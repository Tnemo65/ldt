...
dashboard = {
    "dashboard": {
        ...
        "panels": [
            ...
            {
                "id": 3, "title": "PostgreSQL Connections", "type": "stat",
                "gridPos": {"h": 4, "w": 4, "x": 8, "y": 0},
                "targets": [{"expr": 'pg_stat_database_numbackends{datname="dq_pipeline"}', "legendFormat": "Connections", "refId": "A"}]
            },
            {
            ...
            {
                "id": 20, "title": "PostgreSQL Active Connections", "type": "timeseries",
                "gridPos": {"h": 5, "w": 8, "x": 0, "y": 10},
                "targets": [{"expr": 'pg_stat_database_numbackends{datname="dq_pipeline"}', "legendFormat": "Connections", "refId": "A"}]
            },
            {
                "id": 21, "title": "PG Query Performance", "type": "timeseries",
                "gridPos": {"h": 5, "w": 8, "x": 8, "y": 10},
                "targets": [
                    {"expr": 'rate(pg_stat_database_tup_fetched{datname="dq_pipeline"}[1m])', "legendFormat": "Rows Fetched/s", "refId": "A"},
                    {"expr": 'rate(pg_stat_database_tup_returned{datname="dq_pipeline"}[1m])', "legendFormat": "Rows Returned/s", "refId": "B"}
                ]
            },
            {
            ...
            {
                "id": 42, "title": "DB Size Growth", "type": "timeseries",
                "gridPos": {"h": 5, "w": 8, "x": 16, "y": 20},
                "targets": [{"expr": 'rate(pg_stat_database_dbsize{datname="dq_pipeline"}[5m])', "legendFormat": "DB Size Rate", "refId": "A"}]
            }
        ]
    },
    "overwrite": True,
