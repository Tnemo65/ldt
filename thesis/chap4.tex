...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.98\textwidth]{figs/system-architecture.png}}
...
\textbf{Why Offline Preprocessing is CRITICAL:}
...
Without offline preprocessing, the model would train on 3.07M raw records containing 79,223 physical violations (2.58\%). These violations include:
\begin{itemize}
    \item 12,456 records with fare $\leq$ 0 (e.g., fare = -\$15.00)
...
The model would learn: ``Fare = -\$15.00 appears 12,456 times in training data, therefore it is a normal pattern.'' When deployed to production, the model would assign low anomaly scores to fraudulent negative-fare trips, causing them to pass undetected (False Negatives).

\textbf{Empirical Impact (Prototype Testing):}
\begin{itemize}
    \item \textbf{Model trained on raw data:} FNR=68.2\%, FPR=23.4\% (unusable -- misses 2/3 of fraud, flags 1/4 of normal trips)
    \item \textbf{Model trained on ultra-clean data:} FNR=7.8\%, FPR=5.0\% (production-grade)
\end{itemize}
...
CA-DQStream solves this via a \textbf{3-Stage Sequential Funnel} applied \emph{offline} to the training dataset (January 2024, 3.07M raw records):
...
This removes \textbf{2.58\%} of raw data (79,223 records flagged as physical impossibilities).
...
\begin{equation}
\text{outlier} \iff x < Q_1 - 3 \times \text{IQR} \quad \text{OR} \quad x > Q_3 + 3 \times \text{IQR}
\end{equation}

Applied to: fare\_amount, trip\_distance, trip\_duration.

This removes an additional \textbf{0.92\%} of data (27,613 records flagged as statistical outliers -- e.g., \$5,000 negotiated fares, 200-mile trips).
...
\hline
Raw data & 3,066,766 & --- \\
After physical filter (Stage 1) & 2,987,543 & 2.58\% \\
After IQR outlier removal (Stage 2) & 2,959,930 & 0.92\% \\
\textbf{Ultra-clean baseline (Stage 3)} & \textbf{2,959,930} & \textbf{3.48\% total} \\
\hline
\end{tabular}
\end{center}

The resulting 2.96M \textbf{ultra-clean baseline} records have:
\begin{itemize}
    \item \textbf{Zero physical violations:} fare $>$ 0, distance $>$ 0, speed $<$ 100 mph (79,223 violations removed)
    \item \textbf{Zero data entry errors:} valid passenger counts, timestamps (51,610 errors removed)
    \item \textbf{Minimal statistical outliers:} removed via 3$\times$IQR (27,613 outliers removed)
    \item \textbf{Verified quality:} Null rate=0.012\%, Violation rate=0.000\% (both $<$ 0.5\% threshold)
\end{itemize}

This is the \textbf{pristine dataset} on which the Cold-Start Model is trained. The term ``ultra-clean'' is not marketing language -- it is a quantifiable property: the dataset has passed 3 stages of validation and contains zero records that would fail Schema Filter or Business Rules checks.
...
\hline
Total records & 3,066,766 & 2,959,930 \\
Physical violations & 79,223 (2.58\%) & 0 (0.00\%) \\
Null rate & 4.2\% & 0.012\% \\
...
The Hybrid K-Means iForestASD model is trained \emph{offline} on the 2.96M ultra-clean records using scikit-learn and River libraries:
...
% ================================================
% 4.2 Four-Layer Processing Pipeline
% ================================================
...
\textbf{CA-DQStream Solution:} 4D Context-Aware Threshold Matrix (Chapter 3.2) assigns different thresholds to different contexts, reducing FPR from 38.7\% to 4.2\% (9.2$\times$ improvement).
...
\textbf{Problem:} Business rules (Layer 2a Canary) catch only 13.5\% of anomalies. The remaining 86.5\% require ML-based multivariate detection.
...
\textbf{Problem:} Static models trained on January 2024 degrade by March 2024 (FPR increases from 4.2\% to 12.8\% due to seasonal drift).

\textbf{CA-DQStream Solution:} Intelligent Evolution Controller (Chapter 3.4) monitors meta-stream via ADWIN and selects from 4 adaptation strategies, resolving 78\% of drift via fast incremental updates (seconds, not hours).
...
\hline
Extreme heterogeneity (3.4$\times$ variance) & Global thresholds: FPR=38.7\% & 4D Context-Aware Thresholds: FPR=4.2\% (9.2$\times$ better) \\
\hline
Multivariate fraud (86.5\% of anomalies) & Business rules catch only 13.5\% & Hybrid K-Means iForestASD: Recall=92.2\% \\
\hline
Concept drift (seasonal + events) & Static models degrade (FPR 4.2\% $\to$ 12.8\%) & IEC 4-strategy adaptation: 78\% resolved in seconds \\
\hline
\end{tabular}
\end{center}
...
Branch 2 of the Rendezvous pipeline is the \textbf{Complex Model}: Hybrid K-Means iForestASD (detailed in Chapter 3, Section 3.5) with context-aware spatio-temporal feature engineering. This branch detects multivariate anomalies that business rules cannot express.
...
9 & hour\_sin & Temporal & $\sin(2\pi \cdot hour / 24)$ \\ \hline
10 & hour\_cos & Temporal & $\cos(2\pi \cdot hour / 24)$ \\ \hline
11 & dow\_sin & Temporal & $\sin(2\pi \cdot dow / 7)$ \\ \hline
12 & dow\_cos & Temporal & $\cos(2\pi \cdot dow / 7)$ \\ \hline
13 & pickup\_grid\_x & Spatial & $\lfloor PULocationID / 10 \rfloor$ \\ \hline
14 & pickup\_grid\_y & Spatial & PULocationID mod 10 \\ \hline
...
16 & fare\_per\_mile\_ratio & Normalized & fare\_per\_mile / baseline\_fpm\_context \\ \hline
17 & implied\_speed\_ratio & Normalized & speed\_mph / baseline\_speed\_context \\ \hline
18 & fare\_per\_minute\_ratio & Normalized & fare\_per\_minute / baseline\_fpmm\_context \\ \hline
19 & tip\_ratio\_normalized & Normalized & (tip / fare) / baseline\_tip\_ratio\_context \\ \hline
20 & passenger\_density\_ratio & Normalized & (passengers / distance) / baseline\_density \\ \hline
21 & distance\_log\_ratio & Transformed & log(1 + dist) / baseline\_log\_dist\_context \\ \hline
\end{longtable}
...
\begin{itemize}
    \item \textbf{Baseline (15D absolute features)}: Recall=81.5\%, FPR=63.6\% (unacceptable -- 2 out of 3 normal trips flagged)
    \item \textbf{Enhanced (21D with ratio features)}: Recall=92.2\%, FPR=5.0\% (\textbf{12.7$\times$ FPR improvement})
    \item \textbf{Variance reduction}: $\sigma_{\text{ratio}} / \sigma_{\text{absolute}} = 0.01$--$0.1$ (10--100$\times$ lower)
\end{itemize}
...
Zero-Downtime Model Update Protocol:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Flink Broadcast State delivers new model to all IFScoringOperator instances (zero-downtime hot-swap).
\end{itemize}
...
This retry mechanism ensures that temporary failures (e.g., MLflow pod restart, network timeout) do not cause permanent loss of IEC adaptation actions. The exponential backoff prevents retry storms during sustained outages.
...
This single-node configuration is validated for throughput and latency benchmarks in Chapter 5.
...
Additionally, connection establishment time is eliminated for pooled connections (no TCP handshake, no authentication, no backend process fork), reducing p99 write latency by 95\% (from 450ms to 22ms).


% ================================================
% 4.5 MLOps Integration
% ================================================
...
\begin{itemize}
    ...
    \item \textbf{Rolling deployment:} Flink savepoints enable zero-downtime pipeline upgrades
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Optimization Techniques:}
        \begin{itemize}
            ...
            \item PgBouncer connection pooling: 95\% reduction in connection overhead
            \item Broadcast State: zero-downtime model updates
...
The next chapter (Chapter 5) presents experimental validation of the four core innovations through 11 experiments spanning 72M records across 26 months.
