...
Before deploying the Flink pipeline, CA-DQStream requires a \textbf{Cold-Start Model} trained on ultra-clean baseline data. This offline preprocessing workflow ensures the initial Hybrid K-Means sHST model learns from \textbf{sterile data}, not contaminated by physical impossibilities.
...
Remove statistical outliers using Interquartile Range (IQR) method applied \textbf{per Context Cell} (not globally):

\begin{equation}
x \in \text{outlier} \iff x < Q_1 - 3 \times \mathrm{IQR}_{\text{cell}} \quad \text{or} \quad x > Q_3 + 3 \times \mathrm{IQR}_{\text{cell}}
\end{equation}

Applied per context cell to: fare\_amount, trip\_distance, trip\_duration. The 3$\times$IQR filter is applied per Context Cell (not globally), preserving cell-specific legitimate high values (e.g., \$5,000 negotiated fares in the Negotiated ratecode context cell) while removing statistical anomalies. This ensures sHST learns the full contextual distribution.
...
The Hybrid K-Means sHST model is trained \emph{offline} on the 2.97M ultra-clean records using scikit-learn and River libraries:
...
\begin{enumerate}
    ...
    \item \textbf{sHST Trees:} Train 100 isolation trees per cluster on 21D enhanced features (15D base + 6D ratio features). Contamination parameter set to 0.02 (2\% expected anomaly rate in production).
    ...
    \item \textbf{Model Serialization:} Export K-Means clusterer, sHST trees, threshold matrix, and scaler to joblib pickle format.
\end{enumerate}
...
\textbf{CA-DQStream Solution:} Hybrid K-Means sHST (Chapter 3.5) detects joint-distribution anomalies by learning the normal correlation structure \emph{within each context cluster}.
...
\hline
Multivariate fraud (complex anomaly patterns) & Business rules catch single-feature violations only & Hybrid K-Means sHST: Multivariate anomaly detection \\
\hline
...
Branch 2 of the Rendezvous pipeline is the \textbf{Complex Model}: Hybrid K-Means sHST (detailed in Chapter 3, Section 4.5) with context-aware spatio-temporal feature engineering. This branch detects multivariate anomalies that business rules cannot express.
...
The Hybrid K-Means sHST model (Chapter 3.5) detects these by learning the normal joint distribution of features within each context cluster.
...
\caption{21-dimensional enhanced feature vector for Hybrid K-Means sHST.}
...
\begin{enumerate}
    ...
    \item Atomic model swap in PyFlink worker process memory (pipeline continues uninterrupted)
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Data Routing (Data Plane):} Merges results from both branches. Violation records (from Business Rules or sHST) are emitted to \texttt{dq-dirty-data} Kafka topic (written to PostgreSQL dirty\_data table). Clean records are emitted to \texttt{dq-clean-data}.
...
\begin{itemize}
    ...
    \item $\Delta$\_score(t) = (|violation\_rate(t) $-$ anomaly\_rate(t)|) / (violation\_rate(t) + anomaly\_rate(t) + $\epsilon$) \quad (normalized divergence)
\end{itemize}

The sixth metric, \textbf{$\Delta$\_score} (Concept Uncertainty), is the normalized divergence between the violation rate detected by Business Rules (deterministic, static) and the anomaly rate detected by sHST (probabilistic, adaptive). The normalized formulation addresses calibration assumptions about the correlation between violation and anomaly rates. A warm-up period allows the system to learn baseline correlation patterns before activating drift detection. This metric is the most sensitive indicator of concept drift:
...
anomaly\_rate & 0.001 & 1-minute & Retrain Hybrid K-Means sHST, broadcast via Broadcast State \\ \hline
...
\begin{itemize}
    \item Trigger Hybrid K-Means sHST retraining on MLflow with new training window.
...
\begin{enumerate}
    \item \texttt{POST /api/retrain} --- Triggers full Hybrid K-Means sHST retraining on MLflow. Loads a recent training window from PostgreSQL, retrains the model asynchronously (non-blocking), and publishes the new model to the \texttt{if-model-updates} Kafka topic for Flink Broadcast State consumption.
...
\begin{itemize}
    ...
    \item \textbf{Immediate retry:} Retry with exponential backoff (1min $\to$ 2min $\to$ 4min, max 3 retries).
    \item \textbf{After 2 failed retries:} Emit alert to monitoring system. Continue processing with stale model but flag all Layer 3 scores as \texttt{DRIFT\_SUSPECTED}.
    \item \textbf{After 3 failed retries:} Switch to Strategy 2 (Switching) if available -- use rule-based fallback model.
    \item \textbf{Lock release:} After $T_{\text{lock}} = 15$ minutes, system automatically releases lock and attempts update regardless of previous state. This prevents the retry storm problem while ensuring eventual recovery.
...
\textbf{Exactly-Once Semantics:} CA-DQStream achieves end-to-end exactly-once through two complementary mechanisms: (1) \textbf{Flink Checkpointing:} Periodic state snapshots (45s interval) ensure operator state can be restored on failure. (2) \textbf{Kafka Transactions:} The Flink Kafka producer uses transactional writes with \texttt{transaction.timeout.ms = 10 minutes}. On checkpoint recovery, in-flight transactions are aborted and replayed idempotently. This is \emph{not} a traditional Two-Phase Commit (2PC) distributed transaction -- there is no blocking coordinator; throughput impact is limited to checkpoint barrier alignment phase (approximately 200-400ms per 45s interval, less than 1\% overhead).
    ...
    \item \textbf{Broadcast State:} Distributes Hybrid K-Means sHST model to all parallel operator instances via Kafka-to-Python-worker delivery, loaded in-process for microsecond-access scoring.
...
dq-if-anomalies & Anomalies detected by Hybrid K-Means sHST (Complex branch) \\ \hline
...
baseline\_stats & Rolling baseline statistics for Hybrid K-Means sHST retraining \\ \hline
...
The \texttt{if\_model\_version} metric tracks which Hybrid K-Means sHST model version is currently deployed (incremented on each Broadcast State update). This enables correlating drift events with model version changes.
...
\begin{itemize}
    ...
    \item \textbf{Model Artifact Storage:} Trained Hybrid K-Means sHST model artifacts (serialized Pickle files) are archived in \texttt{models} bucket, enabling model versioning and rollback through MLflow Model Registry.
\end{itemize}
...
\begin{enumerate}
    ...
    \item The DataFrame is passed to the Vectorized UDF, which applies the Hybrid K-Means sHST scoring vectorized across all 500 records at once
...
CA-DQStream uses MLflow \citep{mlflow2024} for ML experiment tracking and model versioning. All Hybrid K-Means sHST model training runs are logged to MLflow with:
...
MLflow Model Registry manages the Hybrid K-Means sHST model lifecycle:
