...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.75\textwidth]{figs/Thesis_Figures/kafka1.PNG}}
\caption{CA-DQStream Kafka topics and consumer groups in the Flink Web UI. The seven topics (taxi-nyc-raw, dq-schema-violations, dq-hard-rule-violations, dq-if-anomalies, dq-anomaly-scores, dq-meta-stream, if-model-updates) with 4 partitions each enable parallel processing across the streaming pipeline.}
...
\begin{enumerate}
    ...
    \item \textbf{Layer 2: Conditional Routing Architecture} --- Records pass through exactly one of two evaluation paths (Canary or Complex) based on rule-based pre-qualification, with results merged at the MetaAggregator.
...
The Hybrid K-Means + sklearn IsolationForest model is trained \emph{offline} on the $\approx$2.86M ultra-clean records using the scikit-learn library:
...
\begin{enumerate}
    ...
    \item \textbf{IsolationForest Trees:} Train 200 isolation trees on 21D enhanced features (15D base + 6D ratio features). Contamination parameter set to 0.001 (0.1\% expected anomaly rate in production, accounting for residual anomalies not caught by Stages 1--2; the contamination level acts as a prior rather than a hard threshold, and the 4D context-aware thresholds in Layer 3 further refine the classification boundary at inference time).
...
\hline
Multivariate fraud (complex anomaly patterns) & Business rules catch single-feature violations only & Hybrid K-Means + sklearn IsolationForest (Chap 4, Sec 4.5) \\
\hline
...
The Complex Branch implements the Hybrid K-Means + sklearn IsolationForest algorithm for multivariate anomaly detection (detailed in Chapter 4, Section 4.5). This section covers the Flink implementation and feature engineering.
...
9 & hour & Temporal & Hour of day (0-23, raw numeric) \\ \hline
10 & day\_of\_week & Temporal & Day of week (0-6, raw numeric) \\ \hline
11 & is\_weekend & Temporal & Binary: 1 if Sat/Sun, 0 otherwise \\ \hline
12 & is\_rush\_hour & Temporal & Binary: 1 if 7-9h or 16-19h \\ \hline
13 & is\_night & Temporal & Binary: 1 if hour $<$ 6 or $>$ 22 \\ \hline
14 & month & Temporal & Month (1-12, raw numeric) \\ \hline
15 & pickup\_grid\_x & Spatial & $g_x(\text{PULocationID})$ via 4$\times$4 grid lookup \\ \hline
16 & pickup\_grid\_y & Spatial & $g_y(\text{PULocationID})$ via 4$\times$4 grid lookup \\ \hline
...
\textbf{Ratio features normalize by global baseline values}, reducing variance 10--100$\times$:

\begin{equation}
\text{fare\_per\_mile\_ratio} = \frac{\text{fare\_per\_mile}}{\text{BASELINE}_{\text{fpm}}}
\end{equation}

where $\text{BASELINE}_{\text{fpm}} = \$2.5$ per mile is a fixed global baseline computed from the training data (January 2024 clean baseline). For example:
\begin{itemize}
    \item JFK airport trip: fare\_per\_mile\_ratio = $52.18 / 2.5 = 20.87$
    \item Standard Manhattan trip: fare\_per\_mile\_ratio = $15.42 / 2.5 = 6.17$
\end{itemize}

A trip with fare\_per\_mile = \$8.00 produces ratio = $8.00 / 2.5 = 3.2$, which is significantly elevated compared to the baseline of 1.0, enabling detection of anomalous trips regardless of trip type.
...
\begin{enumerate}
    \item \texttt{POST /api/retrain} --- Triggers full Hybrid K-Means + sklearn IsolationForest retraining on MLflow. Loads training data, retrains the model asynchronously (non-blocking), publishes the new model artifact to MinIO, and the \texttt{if-model-updates} Kafka topic for Flink Broadcast State consumption.
...
\begin{itemize}
    ...
    \item \textbf{Checkpointing:} Incremental snapshots every 45 seconds to MinIO S3 storage, enabling at-least-once delivery and failure recovery.

\textbf{At-Least-Once Semantics:} CA-DQStream uses at-least-once delivery through two complementary mechanisms: (1) \textbf{Flink Checkpointing:} Periodic state snapshots (45s interval) ensure operator state can be restored on failure. (2) \textbf{Kafka Transactions:} The Flink Kafka producer uses transactional writes with \texttt{transaction.timeout.ms = 10 minutes}. On checkpoint recovery, in-flight transactions are aborted and replayed idempotently. At-least-once is the standard production configuration for streaming systems; exactly-once requires significant throughput overhead from two-phase commit coordination that outweighs the benefit for data quality applications where downstream sinks handle deduplication.
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{figs/Thesis_Figures/flink1.PNG}}
...
Apache Kafka serves as the distributed event streaming backbone. The CA-DQStream pipeline uses 7 topics with 4 partitions each for parallel processing:
...
All pipeline outputs are persisted to MinIO (S3-compatible object storage) via Flink StreamingFileSink in Parquet format. This provides scalable, durable storage with built-in versioning and lifecycle policies.

The storage hierarchy consists of seven buckets:

\begin{longtable}{|m{4cm}|m{9.5cm}|}
\hline
\textbf{Bucket} & \textbf{Description} \\ \hline
cadqstream-raw & Clean records passing all stages, with IF scores and context features \\ \hline
cadqstream-violations & Schema validation rejects (NULL fields, invalid types, duplicates) + business rule violations \\ \hline
cadqstream-anomalies & Isolation Forest detections with anomaly score, priority, and context features \\ \hline
cadqstream-metrics & Aggregated DQ metrics per 1-minute window with IEC drift flags (Parquet) \\ \hline
cadqstream-drift & Drift events and IEC decisions (Parquet) \\ \hline
...
Key features: Parquet format for columnar compression, bucket versioning enabled, lifecycle policies for automatic cleanup of raw data after 7 days.
...
MinIO & minio & s3\_requests\_total, s3\_request\_latency\_seconds, cluster\_capacity\_used \\ \hline
...
\begin{enumerate}
    ...
    \item \textbf{Dashboard 4: System Performance} --- Flink Throughput (records/sec per TaskManager), Kafka Consumer Lag (ms), Processing Latency P50/P95/P99 (heatmap), MinIO S3 Request Latency.
\end{enumerate}
...
\subsection{Scalable Object Storage with MinIO}

MinIO provides high-throughput, S3-compatible object storage that scales horizontally. Flink StreamingFileSink writes Parquet files to MinIO buckets, enabling cost-effective storage with automatic tiering and compression.
