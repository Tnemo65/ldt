...
These innovations work synergistically: the 4D thresholds provide context cells, K-Means clusters data within contexts, sklearn IsolationForest scores anomalies, the Rendezvous pipeline parallelizes execution, and IEC adapts the system when drift occurs.
...
\begin{enumerate}
    \item \textbf{Strategy 1: Continuous Evolution}
        \begin{itemize}
            ...
            \item \textbf{How:} Incrementally update K-Means cluster centroids via \texttt{mini-batch K-Means partial\_fit()} on recent data windows; Isolation Forest trees are rebuilt periodically via Tier 3 when drift persists after centroid updates. Unlike full retraining, centroid-only updates avoid reconstructing the isolation tree ensemble.
...
The IEC applies strategies in a sequential fallback order, exhausting lightweight strategies before resorting to expensive operations: K-Means centroid update first (\texttt{partial\_fit()}), then if drift persists, trigger full Isolation Forest rebuild.
...
\hline
n\_estimators & 100 & 100 & 100 trees provide sufficient ensemble diversity for the 2.97M training dataset; increasing to 200 trees showed marginal improvement (+0.3\% recall) at 2$\times$ training cost in prototype validation. \\
max\_depth & 10 & None & Deeper trees enable better multivariate detection; sklearn default is unlimited depth \\
contamination & 0.02 & 0.05 & 2\% expected anomaly rate in production (prior for score distribution, refined by 4D thresholds) \\
