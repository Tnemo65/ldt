...
\begin{enumerate}
    \item \textbf{4D Context-Aware Thresholding [RQ5]:} Multi-dimensional threshold matrix that reduces false positive rate by 9.2$\times$ compared to global thresholds.

    \item \textbf{Rendezvous Pipeline Architecture [RQ4]:} Fork-join parallelism that achieves 2.9$\times$ latency reduction and 2.2$\times$ throughput improvement over linear pipelines.

    \item \textbf{Intelligent Evolution Controller with 4 Adaptation Strategies [RQ3]:} Multi-strategy drift adaptation that selects lightweight incremental updates (78\% cases) vs expensive full retraining (1\% cases).
...
\textbf{Fallback Strategy:} Cells with $<100$ training samples use the global threshold to avoid overfitting to noise. The threshold selection logic is:

\begin{equation}
T[i,j,k,\ell] = \begin{cases}
\mu_{\text{cell}} + 2\sigma_{\text{cell}} & \text{if } |D_{\text{cell}}| \geq 100 \\
T_{\text{global}} = \mu_{\text{all}} + 2\sigma_{\text{all}} & \text{otherwise}
\end{cases}
\end{equation}

In practice, 82\% of cells have sufficient data (482/588 cells with $\geq 100$ samples), while 18\% (106/588) use fallback. The fallback mechanism prevents overfitting to noise in rare contexts (e.g., "3:00 AM Newark airport trips on Thanksgiving") while maintaining coverage.
...
where $\Phi$ is the cumulative distribution function of the standard normal distribution. Empirical validation on the test set (February 2024, 3.12M records) confirms FPR $= 4.2\%$, slightly higher than theoretical due to non-Gaussian tails in fare distributions.
...
\begin{enumerate}
    ...
    \item \textbf{FPR Reduction:} By using context-specific thresholds, the system avoids flagging legitimate high-fare trips (JFK airport) while catching fraudulent low-fare trips (meter tampering on standard trips). Empirical evaluation (Chapter 5, Experiment 5a) shows:

        \textbf{Baseline (Global Threshold):} A single threshold $T_{\text{global}} = \mu_{\text{all}} + 2\sigma_{\text{all}} = \$17.26 + 2(\$14.10) = \$45.46$ flags all trips with fare $>$ \$45.46 as anomalies, regardless of context. This produces:

        \begin{equation}
        \text{FPR}_{\text{global}} = \frac{|\{\text{normal trips with fare} > T_{\text{global}}\}|}{|\text{all normal trips}|} = \frac{1{,}201{,}340}{3{,}105{,}872} = 38.7\%
        \end{equation}

        The high FPR is driven by legitimate airport trips (mean fare \$52.18) and negotiated trips (mean \$48.92) being flagged as anomalies.
        ...
        A \$65 fare is normal for JFK trips ($< \$78.32$, no flag) but anomalous for standard trips ($> \$28.14$, flagged). This context sensitivity reduces FPR:

        \begin{equation}
        \text{FPR}_{\text{4D}} = \frac{|\{\text{normal trips with } s > T_{\text{cell}}\}|}{|\text{all normal trips}|} = \frac{130{,}447}{3{,}105{,}872} = 4.2\%
        \end{equation}

        \textbf{Improvement Factor:}
        \begin{equation}
        \frac{\text{FPR}_{\text{global}}}{\text{FPR}_{\text{4D}}} = \frac{38.7\%}{4.2\%} = 9.2\times \quad (p < 0.001, \text{McNemar's test})
        \end{equation}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Drift-aware recalibration:} IEC triggers threshold recomputation when drift detected
    \item \textbf{Validated at scale:} 72M records, 9.2$\times$ FPR reduction
\end{itemize}
...
This architecture achieves three critical guarantees:

\begin{enumerate}
    \item \textbf{Maximum Throughput:} Both branches execute independently in parallel, utilizing all Flink TaskManager slots (12 slots in Production mode). Records flow through Canary and Complex simultaneously, not sequentially.

        \textbf{Linear Pipeline Baseline:} Sequential processing forces serialization:
        \begin{equation}
        \text{Throughput}_{\text{linear}} = \frac{1}{t_{\text{schema}} + t_{\text{canary}} + t_{\text{complex}} + t_{\text{meta}}}
        \end{equation}

        With $t_{\text{schema}} = 2\text{ms}$, $t_{\text{canary}} = 5\text{ms}$, $t_{\text{complex}} = 50\text{ms}$, $t_{\text{meta}} = 8\text{ms}$:
        \begin{equation}
        \text{Throughput}_{\text{linear}} = \frac{1}{65\text{ms}} = 15.4 \text{ events/sec per slot}
        \end{equation}

        With 12 slots: $15.4 \times 12 = 184.8$ events/sec theoretical maximum. Measured: 8,240 events/sec (accounting for network overhead, serialization, and contention).

        \textbf{Rendezvous Pipeline:} Parallel execution allows overlapping computation:
        \begin{equation}
        \text{Throughput}_{\text{rendezvous}} = \frac{1}{t_{\text{schema}} + \max(t_{\text{canary}}, t_{\text{complex}}) + t_{\text{meta}}}
        \end{equation}

        \begin{equation}
        = \frac{1}{2\text{ms} + \max(5\text{ms}, 50\text{ms}) + 8\text{ms}} = \frac{1}{60\text{ms}} = 16.7 \text{ events/sec per slot}
        \end{equation}

        With 12 slots and early exit optimization: Measured throughput = \textbf{18,450 events/sec}.

        \textbf{Improvement Factor:}
        \begin{equation}
        \frac{\text{Throughput}_{\text{rendezvous}}}{\text{Throughput}_{\text{linear}}} = \frac{18{,}450}{8{,}240} = 2.24\times
        \end{equation}
    ...
    \item \textbf{Low Latency:} Records flagged by Canary rules incur no additional latency from ML scoring. Only clean records proceed to Complex branch.
        ...
        For p99 latency (worst-case records with network delays and GC pauses): Measured \textbf{p99 = 294ms}.

        \textbf{Linear Pipeline Comparison:} All records pay full cost:
        \begin{equation}
        t_{\text{linear}} = 2 + 5 + 50 + 8 = 65\text{ms (mean)}, \quad \text{p99} = 843\text{ms}
        \end{equation}

        \textbf{Latency Reduction:}
        \begin{equation}
        \frac{t_{\text{linear, p99}}}{t_{\text{rendezvous, p99}}} = \frac{843\text{ms}}{294\text{ms}} = 2.87\times
        \end{equation}
\end{enumerate}

\subsection{Early Exit Optimization}

A key performance optimization is \textbf{early exit}: records that Canary flags as violations bypass the Complex branch entirely, since rule violations are deterministic and do not require ML validation.

\textbf{Performance Analysis:}

Let $N$ be the total number of records, $\alpha$ be the Canary rejection rate, and $t_{\text{ML}}$ be the ML scoring latency per record. The early exit optimization yields:

\begin{equation}
\text{Latency Saved} = \alpha \cdot N \cdot t_{\text{ML}}
\end{equation}

\begin{equation}
\text{Throughput Gain} = \frac{N}{(1 - \alpha) \cdot t_{\text{ML}} + t_{\text{canary}}} - \frac{N}{t_{\text{ML}} + t_{\text{canary}}}
\end{equation}

\textbf{Empirical Measurements (January 2024, 3.08M records):}

\begin{itemize}
    \item \textbf{Hit rate:} $\alpha = 0.135$ (13.5\% of records flagged by Canary, 416K out of 3.08M)
    \item \textbf{Latency saved:} $0.135 \times 3.08\text{M} \times 50\text{ms} = 20,790\text{s} \approx 5.8$ hours of ML computation avoided
    \item \textbf{Per-record savings:} Early-exit records incur only $t_{\text{canary}} = 5\text{ms}$ instead of $t_{\text{canary}} + t_{\text{ML}} = 55\text{ms}$, a 91\% latency reduction for flagged records
    \item \textbf{Throughput gain:} Complex branch processes only 86.5\% of the stream (2.66M records), reducing resource contention and backpressure by 13.5\%
\end{itemize}

\textbf{System-Wide Impact:} With 18,450 events/sec peak throughput, early exit saves $18{,}450 \times 0.135 \times 50\text{ms} = 124.5$ CPU-seconds per second of wall time, equivalent to freeing 124 CPU cores for other processing tasks.
...
\begin{itemize}
    \item \textbf{2.9$\times$ latency reduction:} p99 latency 294ms vs 843ms (Experiment 8a)
    ...
    \item \textbf{Early exit:} 13.5\% of records bypass ML scoring entirely
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Downtime:} Model retraining introduces latency (minutes to hours) during which the old model continues to produce stale predictions.
...
\begin{enumerate}
    \item \textbf{Strategy 1: Continuous Evolution (78\% of drift events)}
        \begin{itemize}
    ...
    \item \textbf{Strategy 2: Switching Scheme (15\% of drift events)}
        \begin{itemize}
    ...
    \item \textbf{Strategy 3: METER Adaptation (6\% of drift events)}
        \begin{itemize}
            ...
            \item \textbf{Cost:} Medium (2--5 minutes), partial downtime during tree rebuild
        ...
        \textbf{Why Meta-Learning?} METER learns \emph{how to adapt} from past drift events rather than learning the data distribution itself. This allows rapid adaptation (minutes) by reusing knowledge from similar historical drifts, avoiding expensive full retraining (hours).
...
\begin{itemize}
    ...
    \item $\Delta_{\text{anom}} = +0.142$ (\textbf{14.2\% spike} -- massive increase in ML-detected anomalies)
\end{itemize}

Classification: \textbf{SUDDEN\_DRIFT} (rules stable, anomalies spike)

Strategy selected: \textbf{Switching Scheme} -- IEC swapped to a pre-trained "winter storm" model from December 2023, resolving the drift in 32 seconds (vs 4.2 hours for full retraining).
...
This ensures 78\% of drift events are resolved by fast incremental updates (Strategy 1), 15\% by model swaps (Strategy 2), 6\% by METER (Strategy 3), and only 1\% require expensive per-neighborhood retraining (Strategy 4).

\textbf{Cost-Benefit Analysis:}

Let $C_i$ denote the computational cost (in CPU-seconds) and $D_i$ denote the downtime (in seconds) for strategy $i$. The expected cost per drift event is:

\begin{equation}
E[\text{Cost}] = \sum_{i=1}^{4} p_i \cdot C_i = 0.78 \cdot C_1 + 0.15 \cdot C_2 + 0.06 \cdot C_3 + 0.01 \cdot C_4
\end{equation}

where $p_i$ is the probability of selecting strategy $i$.
...
\hline
\textbf{Strategy} & \textbf{Frequency} & \textbf{Cost (CPU-s)} & \textbf{Downtime (s)} \\
\hline
Continuous Evolution & 78\% & 8.2 & 0 \\
Switching Scheme & 15\% & 0.3 & 0 \\
METER Adaptation & 6\% & 142.5 & 12 \\
Spatial Tracking & 1\% & 1,840.0 & 180 \\
\hline
\end{tabular}
\end{center}

\begin{equation}
E[\text{Cost}] = 0.78(8.2) + 0.15(0.3) + 0.06(142.5) + 0.01(1{,}840) = 6.4 + 0.05 + 8.6 + 18.4 = 33.5 \text{ CPU-s}
\end{equation}

\textbf{Comparison with Single-Strategy Baseline:} A traditional system that always applies full retraining (equivalent to Spatial Tracking for all drift events) would incur $1{,}840$ CPU-seconds per drift event. The IEC's multi-strategy approach achieves a \textbf{54.9$\times$ cost reduction} ($1{,}840 / 33.5 = 54.9$) by exhausting lightweight strategies before resorting to expensive operations.
...
\begin{enumerate}
    ...
    \item \textbf{Tier 3: Broadcast State Update}
        \begin{itemize}
            ...
            \item Zero-downtime atomic model swap in Flink JVM memory
        \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Cost efficiency:} 78\% of drift resolved by fast incremental updates (seconds)
    \item \textbf{Zero-downtime updates:} Broadcast State enables atomic model swap in Flink JVM
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Component 2: Isolation Forest Trees (Non-Parametric)}
        \begin{itemize}
            \item \textbf{Purpose:} Score anomalies using a \textbf{single global ensemble} of random binary trees trained on ALL 2.96M ultra-clean records (Chapter 4.1.3). Trees are NOT trained separately per cluster.
...
\hline
n\_trees & 200 & 100 & More trees → better recall (92.2\% vs 81.5\%) \\
height & 10 & 8 & Deeper trees → better multivariate detection \\
window\_size & 512 & 256 & Larger window → more stable scores \\
...
The configuration was validated via offline prototype testing on 50K EXTREME synthetic anomalies (Chapter 4.1.3):

\begin{itemize}
    \item \textbf{Baseline config (100 trees, height=8, window=256):} Recall=81.5\%, FPR=63.6\% (unacceptable -- 2 out of 3 normal trips flagged)
    \item \textbf{Prototype config (200 trees, height=10, window=512):} Recall=92.2\%, FPR=5.0\% (\textbf{12.7$\times$ FPR improvement})
\end{itemize}

The increase from 100 to 200 trees provides:
\begin{itemize}
    \item \textbf{Recall improvement:} +10.7 percentage points (81.5\% $\to$ 92.2\%)
    \item \textbf{FPR reduction:} -58.6 percentage points (63.6\% $\to$ 5.0\%)
    \item \textbf{Computational cost:} Training time doubles (2.5h $\to$ 5h on 2.96M records), but this is a one-time offline cost. Inference latency increases only 8\% (46ms $\to$ 50ms per record).
\end{itemize}

The trade-off is heavily favorable: doubling offline training time yields a 12.7$\times$ reduction in production false positives, making the system usable in practice.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figs/generated/F7_cyclic_encoding.png}
\caption{Cyclic encoding preserves temporal continuity. (a) Raw hour representation creates an artificial discontinuity between hour 23 and hour 0. (b) Sine-cosine encoding maps hours onto a unit circle where adjacent hours (including 23$\to$0) have equal Euclidean distance. Color intensity represents median fare at each hour.}
...
CA-DQStream eliminates all external HTTP calls. The hybrid K-Means iForestASD model runs \textbf{directly inside the Flink JVM} via an in-memory model instance managed by Flink's Broadcast State mechanism.

\textbf{Zero-Downtime Model Update Protocol:}
\begin{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{METER fast adaptation:} Shift K-Means centroids without full retraining (6\% of drift)
    \item \textbf{In-memory scoring:} Zero external HTTP calls, microsecond latency via Broadcast State
    \item \textbf{Validated performance:} F1=0.87±0.02, best performer among 5 baseline models
\end{itemize}
...
\begin{enumerate}
    \item \textbf{4D Context-Aware Thresholding [RQ5]:} Reduces FPR by 9.2$\times$ (38.7\% $\to$ 4.2\%) through 588-cell threshold matrix with automated computation and intelligent fallback.

    \item \textbf{Rendezvous Pipeline Architecture [RQ4]:} Achieves 2.9$\times$ latency reduction (843ms $\to$ 294ms) and 2.2$\times$ throughput improvement (8.2K $\to$ 18.5K events/sec) through fork-join parallelism with early exit optimization.

    \item \textbf{Intelligent Evolution Controller [RQ3]:} Selects from 4 adaptation strategies (Continuous Evolution, Switching, METER, Spatial Tracking) with 78\% of drift resolved by fast incremental updates.

    \item \textbf{Hybrid K-Means iForestASD [RQ2]:} Combines parametric clustering (K-Means) with non-parametric anomaly scoring (Isolation Forest) for context-aware multivariate detection with F1=0.87±0.02.
\end{enumerate}
