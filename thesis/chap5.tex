...
\textbf{Hardware:} AMD Threadripper 64-core CPU, 50GB RAM, PCIe 4.0 NVMe SSD
...
\textbf{Deployment Mode:} Single-node containerized deployment via Docker Compose with 3 Flink TaskManagers (12 slots total), parallelism = 12.
...
The benchmark focuses on validating the incremental contribution of each architectural component rather than exhaustive comparison. We evaluate \textbf{5 focused variants}:
...
\begin{enumerate}
    ...
    \item \textbf{Baseline 2 -- Ratio iForest:} 21D ratio features + global 96th percentile threshold (validates ratio feature value -- prototype: Recall=92.2\%, FPR=5.0\%)
    \item \textbf{Proposed -- Context-Aware iForest:} 21D ratio features + per-cluster adaptive thresholds (full system -- validates per-cluster threshold value)
    \item \textbf{Opponent 1 -- ARF:} Adaptive Random Forest with 200 trees (established streaming ensemble)
    \item \textbf{Opponent 2 -- LODA:} Lightweight Online Detector of Anomalies (fast baseline for throughput comparison)
\end{enumerate}

\textbf{Excluded Algorithms and Justification:}
\begin{itemize}
    \item \textbf{One-Class SVM:} Removed after achieving 0\% Recall in prototype testing on EXTREME synthetic anomalies (10--30$\times$ fare multipliers, 150--300 mph speeds). SVM's RBF kernel failed to generalize to context-specific impossibilities.
    \item \textbf{LOF (Local Outlier Factor):} Not suitable for streaming -- requires full dataset in memory for k-NN distance computation. Would need to load 3M records per batch.
    \item \textbf{ExactStorm, HSTrees:} Redundant with Basic iForest -- HSTrees is River's implementation of Isolation Forest.
\end{itemize}

This focused 5-variant comparison isolates the value of ratio features (Baseline 1 vs 2) and per-cluster thresholds (Baseline 2 vs Proposed).
...
\textbf{Objective:} Benchmark the proposed Context-Aware iForestASD against simpler baselines and established streaming algorithms to quantify the incremental value of (1) ratio features and (2) per-cluster thresholds.
...
\begin{enumerate}
    ...
    \item \textbf{Proposed -- Context-Aware iForest:} 21D ratio features + per-cluster adaptive thresholds (588 cells from 4D matrix)
    \item \textbf{Opponent 1 -- ARF:} Adaptive Random Forest (200 trees, max\_features=sqrt)
    \item \textbf{Opponent 2 -- LODA:} Lightweight Online Detector of Anomalies (histogram-based projections)
\end{enumerate}
...
Opponent 1 -- ARF & 0.84 & 0.88 & 0.86 & 6.2\% & 78 \\ \hline
Baseline 1 -- Static iForest & 0.42 & 0.82 & 0.56 & 63.6\% & 48 \\ \hline
Opponent 2 -- LODA & 0.78 & 0.75 & 0.77 & 8.4\% & 32 \\ \hline
\end{longtable}

\textbf{Key Findings:}

\begin{enumerate}
    \item \textbf{Proposed Context-Aware is Best Overall:} F1=0.91, FPR=3.8\% (production-grade accuracy)
    ...
    \item \textbf{ARF is Competitive but Slower:} F1=0.86 (5.5\% lower than Proposed), latency=78ms (1.5$\times$ slower)

    \item \textbf{LODA is Fastest but Less Accurate:} F1=0.77 (15.4\% lower than Proposed), latency=32ms (1.6$\times$ faster)
\end{enumerate}

\textbf{Validation of H2:} \checkmark \textbf{SUPPORTED.} Context-Aware iForestASD achieves F1=0.91 (exceeds 0.85 threshold) and FPR=3.8\% (below 5\% target), outperforming all baselines.
...
Figure~\ref{fig:benchmark_difficulty} presents the benchmark comparison across three difficulty levels (Easy, Medium, Hard) and the FPR panel.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figs/generated/F10_benchmark_by_difficulty.png}
\caption{5-variant benchmark across three anomaly difficulty levels. The Proposed (Context-Aware) variant achieves the highest F1 on Easy anomalies (0.943) and the lowest FPR overall (2.9\%). LOF achieves higher recall on Hard anomalies but at the cost of 60\% higher FPR (4.7\% vs 2.9\%).}
\label{fig:benchmark_difficulty}
\end{figure}

Figure~\ref{fig:model_radar} provides a holistic view of all variants across four metrics, highlighting that the Proposed approach offers the best balance between detection accuracy and false positive control.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{figs/generated/F13_model_comparison_radar.png}
\caption{Multi-metric radar comparison of 5 model variants. The Proposed (Context-Aware) variant (green, bold) achieves the highest 1-FPR (lowest false positive rate) while maintaining competitive F1, Recall, and Precision. LOF (orange) has the highest Recall but at the cost of elevated FPR.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figs/generated/F11_fpr_comparison.png}
\caption{FPR Comparison: Global vs 4D Context-Aware Thresholds. Global thresholds catastrophically fail on airport trips (51.2\% FPR for JFK, 68.9\% for Newark), while 4D context-aware thresholds reduce FPR to 4.2\% overall (9.2$\times$ improvement).}
...
\begin{enumerate}
    \item \textbf{RQ1 (Multi-Layer Coverage):} ✓ VALIDATED. Multi-layer architecture detects 52\% more violations than schema-only (473K vs 311K), with 79.7\% of IF anomalies unique to ML layer.

    \item \textbf{RQ2 (Hybrid K-Means iForestASD):} ✓ VALIDATED. F1=0.87, outperforming baselines by 8.8--28.8\%. Proves value of K-Means clustering for context-aware multivariate detection.

    \item \textbf{RQ3 (IEC Multi-Strategy Adaptation):} ✓ VALIDATED. Detects drift within 21 hours (avg), restores FPR within 5.2 days using lightweight strategies in 93\% of cases.

    \item \textbf{RQ4 (Rendezvous Architecture):} ✓ VALIDATED. Achieves 2.9$\times$ latency reduction (843ms $\to$ 294ms) and 2.2$\times$ throughput improvement (8.2K $\to$ 18.5K events/sec).

    \item \textbf{RQ5 (4D Context-Aware Thresholds):} ✓ VALIDATED. Reduces FPR by 9.2$\times$ (38.7\% $\to$ 4.2\%), with 10$\times$ improvement on airport trips specifically.
\end{enumerate}
