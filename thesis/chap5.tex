...
% ================================================
\section{RQ2: Does Hybrid K-Means iForestASD Outperform Baselines? [Exp 3, 4]}
...
\begin{enumerate}
    \item Inject 4 types of synthetic multivariate anomalies into validation set:
        ...
        \end{itemize}
    \item Train Hybrid K-Means iForestASD on Jan 2024 training set (2.97M records)
...
\textbf{Hypothesis 3 (H3):} The IEC detects drift within one monitoring window and selects appropriate adaptation strategies based on drift characteristics, resolving most drift events via lightweight incremental updates.
...
\textbf{Validation of H3:} \checkmark \textbf{SUPPORTED.} IEC detects drift within one monitoring window and selects appropriate adaptation strategies, resolving most drift events via lightweight incremental updates without full retraining.
...
\hline
\textbf{Architecture} & \textbf{Avg Latency (ms)} & \textbf{p50 Latency (ms)} & \textbf{Throughput (events/sec)} & \textbf{Early Exit (\%)} \\ \hline
...
\textbf{Key Findings:}
\begin{itemize}
    \item \textbf{Average Latency Reduction:} Average latency drops from 65ms to 54ms (1.20$\times$) due to early exit optimization --- 13.5\% of records flagged by Canary bypass ML scoring entirely
...
\caption{FPR Comparison: Global vs. 4D Context-Aware Thresholds. Global thresholds catastrophically fail on airport trips (51.2\% FPR for JFK, 68.9\% for Newark), while 4D context-aware thresholds reduce FPR to below 6\% across all contexts. The bar chart shows the 9.2$\times$ FPR reduction (38.7\% $\to$ 4.2\%) achieved by context-aware thresholding over global thresholds. The weighted average FPR reduction accounting for class imbalance across 4 trip type categories is 4.9$\times$, reflecting the higher FPR rates in rare special ratecode categories.\label{fig:exp5a_fpr_comparison}}
\end{figure}
...
\item \textbf{RQ1 (Multi-Layer Coverage):} \checkmark VALIDATED. Multi-layer architecture detects 52\% more violations than schema-only (473K vs 311K), with complementary detection across layers.
    ...
    \item \textbf{RQ2 (Hybrid K-Means iForestASD):} \checkmark VALIDATED. Proposed achieves F1=0.91, FPR=3.8\% (below 5\% target), meeting all production requirements.

    \item \textbf{RQ3 (IEC Multi-Strategy Adaptation):} \checkmark VALIDATED. Detects drift within one monitoring window, selecting appropriate strategies based on drift characteristics.
