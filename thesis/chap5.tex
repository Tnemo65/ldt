...
\begin{itemize}
    ...
    \item Python 3.10.12 with River 0.21.0 (sHST, ADWIN)
\end{itemize}
...
% ================================================
\section{RQ2: Does Hybrid K-Means sHST Outperform Baselines? [Exp 3, 4]}

\textbf{Research Question 2:} Does the Context-Aware sHST (21D ratio features + per-cluster thresholds) outperform simpler baseline approaches and established streaming algorithms on multivariate taxi trip anomalies?

\textbf{Hypothesis 2 (H2):} The proposed Context-Aware sHST will achieve F1 $\geq$ 0.85 and FPR $<$ 5\% (specifically 3.8--4.2\% depending on evaluation period) compared to 38.7\% for global thresholds, demonstrating the incremental value of (1) ratio features and (2) per-cluster thresholds over simpler baselines.
...
\textbf{Objective:} Validate that Hybrid K-Means sHST detects multivariate anomalies that business rules cannot catch.
...
\begin{enumerate}
    \item Inject 4 types of synthetic multivariate anomalies into validation set, following contextual constraints to preserve physical fidelity:
        ...
        \end{itemize}
    \item Train Hybrid K-Means sHST on Jan 2024 training set (2.97M records)
...
\textbf{Objective:} Benchmark the proposed Context-Aware sHST to quantify the incremental value of (1) ratio features and (2) per-cluster thresholds.
...
\begin{enumerate}
    ...
    \item \textbf{Proposed -- Context-Aware sHST:} 21D ratio features + K-Means clustering + 4D context-aware thresholds (full system with all innovations)
\end{enumerate}

\textbf{Comparison with Streaming Anomaly Detection Methods:} We additionally compare CA-DQStream against two state-of-the-art streaming anomaly detection methods:
\begin{itemize}
    \item \textbf{MemStream} (Bhatia et al., 2022): Memory-based architecture for streaming anomaly detection with concept drift handling
    \item \textbf{MStream} (Siddiqui et al., 2019): Streaming anomaly detection with dynamic graph-based clustering
\end{itemize}
All methods evaluated under identical streaming conditions (15,000 events/sec, NYC Taxi dataset). Note: MemStream and MStream focus on anomaly detection in concept drift scenarios; CA-DQStream addresses data quality dimensions within the streaming pipeline. Comparison is provided for reference on ML-based streaming detection capabilities.
...
\begin{enumerate}
    ...
    \item \textit{Ratio features are essential for threshold computation:}
        \begin{itemize}
...
\textbf{Validation of H2:} \checkmark \textbf{SUPPORTED.} Context-Aware sHST achieves F1=0.91 (exceeds 0.85 threshold) and FPR=3.8--4.2\% (below 5\% target), meeting all production targets. This represents a 9.2$\times$ improvement over the global threshold FPR of 38.7\%.
...
\caption{Multi-metric performance of the Proposed Context-Aware sHST model. The proposed method achieves F1=0.91, Recall=0.92, Precision=0.90, and FPR=3.8\%. All metrics meet or exceed the production targets (FPR $<$ 5\%, Recall $\geq$ 75\%).\label{fig:model_radar}}
\end{figure}

\textbf{Conclusion for RQ2:} Hybrid K-Means sHST is the best-performing model for streaming taxi trip anomaly detection, combining context awareness (K-Means) with multivariate scoring (sHST).
...
\begin{enumerate}
    ...
    \item Apply both thresholding strategies to Hybrid K-Means sHST anomaly scores
...
\item \textbf{RQ1 (Multi-Layer Coverage):} \checkmark VALIDATED. Multi-layer architecture detects 52\% more violations than schema-only (473K vs 311K), with complementary detection across layers.
    ...
    \item \textbf{RQ2 (Hybrid K-Means sHST):} \checkmark VALIDATED. Proposed achieves F1=0.91, FPR=3.8\% (below 5\% target), meeting all production requirements.
