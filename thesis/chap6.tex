...
\subsection{Contribution 4: Hybrid K-Means sHST Model [RQ2]}

Basic Isolation Forest lacks context awareness---it cannot distinguish "globally rare but locally normal" patterns. CA-DQStream introduces a \textbf{Hybrid K-Means sHST model} that integrates parametric clustering (K-Means) with non-parametric anomaly scoring (Isolation Forest):
...
\textbf{H2:} Hybrid K-Means sHST achieves F1 $\geq$ 0.85 & \checkmark SUPPORTED & F1=0.91, FPR=3.8\% (below 5\% target), meeting all production requirements \\ \hline
...
The 15D feature vector requires domain knowledge to design. Poor feature choices (e.g., omitting critical temporal or spatial context) degrade Hybrid K-Means sHST performance.
...
\begin{itemize}
    ...
    \item \textbf{Active Learning Loop:} Incrementally retrain Hybrid K-Means sHST with human-labeled examples
...
CA-DQStream demonstrates that DQ rules can be \textbf{learned from data} (4D thresholds, Hybrid K-Means sHST) rather than manually specified. This "data quality as code" paradigm:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Hybrid K-Means sHST:} F1=0.91 through context-aware multivariate anomaly detection
\end{enumerate}
