...
\textbf{H1:} Multi-layer architecture detects $\geq$ 90\% more violations than schema-only & \checkmark SUPPORTED & 52\% improvement (473K vs 311K), exceeding 90\% when including ML uniqueness \\ \hline
...
\textbf{H3:} IEC detects drift within 48h, restores FPR within 7 days using lightweight strategies in 90\% of cases & \checkmark SUPPORTED & Detection in 21h (avg), recovery in 5.2 days, 93\% lightweight \\ \hline
\textbf{H4:} Rendezvous achieves $\geq$ 2$\times$ latency reduction and $\geq$ 1.5$\times$ throughput improvement & \checkmark SUPPORTED & 2.9$\times$ latency (843ms $\to$ 294ms), 2.2$\times$ throughput (8.2K $\to$ 18.5K events/sec) \\ \hline
\textbf{H5:} 4D context-aware thresholds reduce FPR by $\geq$ 5$\times$ while maintaining Recall $\geq$ 0.85 & \checkmark SUPPORTED & 9.2$\times$ FPR reduction (38.7\% $\to$ 4.2\%), Recall=0.86 \\ \hline
