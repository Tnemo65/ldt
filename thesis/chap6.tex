...
\textbf{Impact:}
\begin{itemize}
    \item \textbf{F1=0.91, FPR=3.8\%:} Meets all production targets (FPR $<$ 5\%, Recall $\geq$ 75\%)
...
\textbf{H2:} Hybrid K-Means + sklearn IsolationForest achieves F1 $\geq$ 0.85 & \checkmark SUPPORTED & F1=0.91, FPR=3.8\% (below 5\% target), meeting all production requirements \\ \hline
\textbf{H3:} IEC detects drift within 48 hours (within two 24-hour monitoring windows, where each monitoring window is defined as 1 hour of 1-minute-aggregated meta-stream data) and selects appropriate strategies based on drift characteristics & \checkmark SUPPORTED & Drift detected promptly (avg 21 hours), lightweight strategies resolve most cases \\ \hline
...
\begin{enumerate}
    ...
    \item \textbf{Hybrid K-Means + sklearn IsolationForest:} F1=0.91 through context-aware multivariate anomaly detection
\end{enumerate}
