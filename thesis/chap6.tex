...
\textbf{H2:} Hybrid K-Means iForestASD achieves F1 $\geq$ 0.85 & \checkmark SUPPORTED & F1=0.91, FPR=3.8\% (below 5\% target), meeting all production requirements \\ \hline
\textbf{H3:} IEC detects drift within one monitoring window and selects appropriate strategies based on drift characteristics & \checkmark SUPPORTED & Drift detected promptly, lightweight strategies resolve most cases \\ \hline
...
\textbf{H6:} 4D context-aware thresholds maintain FPR below 5\% compared to 25--50\% for global thresholds (at least 5$\times$ improvement) over 26 months with IEC recalibration & \checkmark SUPPORTED & FPR drift 4.2\% $\to$ 4.9\% (16.7\% increase) vs 4.2\% $\to$ 11.2\% (166.7\%) without IEC, maintaining below 5\% threshold \\ \hline
\end{longtable}
...
\begin{enumerate}
    \item \textbf{4D Context-Aware Thresholding:} 9.2$\times$ FPR reduction (38.7\% $\to$ 4.2\%) through automated multi-dimensional threshold computation
    ...
    \item \textbf{Hybrid K-Means iForestASD:} F1=0.91 through context-aware multivariate anomaly detection
\end{enumerate}
