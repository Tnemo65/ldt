...
Completeness & NULL passenger\_count, NULL RatecodeID & 9.1\% & Schema Validation (NULL checks) \\ \hline
Validity & Negative fare, zero distance, extreme speed & 3.4\% & Business Rules (6 deterministic rules) \\ \hline
Multivariate & Meter tampering, short-trip fraud, traffic manipulation & 2.9\% & Isolation Forest (8-feature scoring) \\ \hline
Uniqueness & Duplicate records from reprocessing & Variable & Flink KeyedState deduplication \\ \hline
Concept Drift & NULL rate increases from 4.3\% to 5.9\% over 26 months & -- & ADWIN on quality meta-streams \\ \hline
\end{longtable}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figs/generated/F2_scatter_anomaly_zones.png}
...
\subsection{Gap 1: No ML Integration for Streaming DQ}

No open-source framework integrates ML-based anomaly detection for streaming DQ. The multivariate anomalies described above (meter tampering, short-trip fraud, traffic manipulation) remain undetected without ML. While Isolation Forest is production-proven at companies like Uber and Grab, no open-source framework provides this capability for streaming data.

\subsection{Gap 2: No Self-Monitoring of the DQ System}

No framework monitors its own output to detect when rule thresholds or ML models have become stale. Our case study shows NULL rates increasing from 4.3\% (January 2024) to 5.9\% (February 2026). Without meta-level monitoring, gradual concept drift goes undetected until the DQ model produces unreliable results.

\subsection{Gap 3: Batch-Oriented DQ Frameworks}

Existing frameworks (Great Expectations, AWS Deequ, Soda Core) operate in batch mode and cannot process streaming data in real time. Stream DaQ (2025) provides streaming support but only supports basic deterministic rules.
...
\subsection{RQ1: Isolation Forest Anomaly Detection}
\textit{``Can Isolation Forest detect multivariate anomalies that business rules miss?''}

\begin{itemize}
    \item \textbf{RQ1a:} What is the Precision/Recall of Isolation Forest anomalies (records not flagged by Layers 1--2)?
    \item \textbf{RQ1b:} What types of anomalies does Isolation Forest detect that rules miss?
    \item \textbf{RQ1c:} Does Isolation Forest retraining via partial\_fit() maintain ML performance over time?
\end{itemize}

\subsection{RQ2: ADWIN Drift Detection}
\textit{``Does ADWIN-based drift detection on quality meta-streams detect degradation promptly?''}

\begin{itemize}
    \item \textbf{RQ2a:} What is the Mean Time To Detect (MTTD) for gradual NULL rate increases?
    \item \textbf{RQ2b:} What is the false positive rate of the drift detector?
    \item \textbf{RQ2c:} Does self-adaptation (threshold recalculation) improve FPR after drift?
    \item \textbf{RQ2d:} What is the appropriate sensitivity parameter $\delta$ for each quality metric?
\end{itemize}

\subsection{RQ3: Business Rules Effectiveness}
\textit{``Do deterministic business rules effectively detect validity violations?''}

\begin{itemize}
    \item \textbf{RQ3a:} What is the coverage of each business rule on the NYC Taxi dataset?
    \item \textbf{RQ3b:} What is the false positive rate of business rules?
    \item \textbf{RQ3c:} How do business rules compare to Great Expectations and Stream DaQ?
\end{itemize}

\subsection{RQ4: Schema Validation Effectiveness}
\textit{``Does schema validation effectively detect completeness violations?''}

\begin{itemize}
    \item \textbf{RQ4a:} What is the NULL rate trend over 26 months?
    \item \textbf{RQ4b:} Does deduplication via Flink KeyedState effectively remove duplicates?
    \item \textbf{RQ4c:} Does Flink State-based dedup handle high-volume streams?
\end{itemize}

\subsection{RQ5: Context-Aware Thresholding Effectiveness}
\textit{``Does 4D context-aware thresholding reduce FPR compared to global thresholds?''}
...
\begin{enumerate}
    \item \textbf{H1:} Isolation Forest detects at least 1\% of total violations not caught by Layers 1--2.
    \item \textbf{H2:} The combined four-stage pipeline achieves FPR below 5\% on the NYC Taxi dataset.
    \item \textbf{H3:} ADWIN detects gradual NULL rate drift within one monitoring window.
    \item \textbf{H4:} Self-adaptation (threshold recalculation) reduces FPR to pre-drift levels within two windows.
    \item \textbf{H5:} Flink KeyedState deduplication achieves dedup rate > 99\% on duplicate records.
    \item \textbf{H6:} 4D context-aware thresholds achieve FPR below 5\% compared to 25--50\% for global thresholds (5--10$\times$ improvement).
\end{enumerate}
