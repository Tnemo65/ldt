...
In the context of the CA-DQStream framework, Kafka serves as the distributed event streaming backbone. NYC TLC Parquet files are ingested via Kafka Producer into the \texttt{taxi-nyc-raw} topic, and the six output/meta topics (\texttt{dq-schema-violations}, \texttt{dq-hard-rule-violations}, \texttt{dq-if-anomalies}, \texttt{dq-anomaly-scores}, \texttt{dq-meta-stream}, and \texttt{if-model-updates}) form the streaming pipeline.
...
The generic Kafka Streams architecture is illustrated in Figure~\ref{fig:kafka_streams_overview} for reference. In production, CA-DQStream deploys the Kafka layer with seven topics as shown in Figure~\ref{fig:kafka_topics_ui}.
