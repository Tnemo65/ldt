...
In the context of the CA-DQStream framework, Kafka serves as the distributed event streaming backbone. NYC TLC Parquet files are ingested via Kafka Producer into the \texttt{taxi-nyc-raw} topic, and the six output/meta topics (\texttt{dq-schema-violations}, \texttt{dq-hard-rule-violations}, \texttt{dq-if-anomalies}, \texttt{dq-anomaly-scores}, \texttt{dq-meta-stream}, and \texttt{if-model-updates}) form the streaming pipeline. The Kafka topics and consumer groups configuration is illustrated in Chapter 5, Figure~\ref{fig:kafka_topics_ui_architect}.
...
Flink provides checkpoint-based state consistency through a combination of state snapshots (checkpoints) and stream replay. These snapshots capture the entire state of the distributed pipeline---recording offsets into input queues as well as state throughout the job graph. When a failure occurs, sources are rewound, state is restored, and processing resumes. Flink's asynchronous and incremental checkpointing algorithm ensures minimal impact on processing latencies. The Flink Web UI displaying the CA-DQStream job execution, task status, and task manager metrics is illustrated in Chapter 5, Figure~\ref{fig:flink_job_execution}.
...
Flink's windowing mechanism enables aggregation over finite sets of events. Windows can be defined by time (tumbling, sliding, session) or count, and can be assigned based on event time or processing time. Figure~\ref{fig:flink_windows} illustrates how Flink partitions unbounded streams into bounded windows for stateful computation.
...
\label{fig:flink_stateful}
\end{figure}

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.9\linewidth]{figs/Thesis_Figures/flink3.PNG}}
\caption{Flink windowing on a data stream. An unbounded stream of events is partitioned into time-based windows (W1, W2, W3, ...) for aggregation and stateful computation. Each window maintains state and emits results independently. Source: Apache Flink Documentation \citep{flinkdocs2024}.}
...
\textbf{Foundational Context-Aware Computing:} The concept of context-awareness originates from \citep{dey2001understanding}, which defines context as any information that can be used to characterize the situation of entities. This foundational work established that context includes not just the data itself, but the circumstances under which data was collected and will be used---a principle directly applicable to data quality thresholding.
...
\textbf{Gap:} No existing streaming DQ framework combines (1) multi-dimensional context-aware thresholding, (2) automated threshold computation from training data, and (3) drift-aware threshold recalibration. CA-DQStream fills this gap with the 4D context-aware threshold matrix that significantly reduces False Positive Rate compared to baseline methods (detailed benchmark results in Chapter 6). The 4D Context-Aware thresholding combined with adaptive ML inference achieves higher precision in distinguishing genuine anomalies from legitimate contextual outliers.

\textbf{Recent Streaming DQ Frameworks:} While batch-oriented DQ frameworks (Great Expectations, AWS Deequ, Soda Core) have dominated the landscape, recent works have extended DQ monitoring to streaming contexts. Stream DaQ (Papastergios \& Gounaris) employs continuous windowing mechanisms with dynamic constraint adaptation. Grab's Coban system uses FlinkSQL for real-time semantic validation through Kafka stream contracts. Rapidash (Liu et al., 2024) addresses complex multi-record violations using spatial indexing. MemStream (Bhatia et al., 2022) utilizes memory-based architecture for streaming anomaly detection. CA-DQStream distinguishes itself through multi-dimensional 4D Context-Aware modeling combined with adaptive ML inference and an Intelligent Evolution Controller for continuous model adaptation.
...
\textbf{Gap:} No existing DQ framework uses Rendezvous architecture for validation. Great Expectations, AWS Deequ, Soda Core, and Stream DaQ all use sequential processing. CA-DQStream applies fork-join parallelism adapted from distributed stream processing paradigms (Flink, Spark Streaming) to the specific domain of data quality validation. While fork-join patterns have been used in general stream processing (e.g., Kappa/Lambda architectures, Grab's Coban system), CA-DQStream is, to our knowledge, the first to apply this pattern specifically to heterogeneous DQ validation stages combining schema checks, business rules, context-aware thresholds, and ML-based anomaly detection within a unified rendezvous architecture.

\subsection{Drift Detection}

Concept drift detection is well-studied in machine learning \citep{gama2014survey, lu2018learning}. \textbf{ADWIN (Adaptive Windowing)} \citep{bifet2007learning, adwin2025} is a state-of-the-art drift detection algorithm. ADWIN maintains a variable-length window $W$ of recent observations and adaptively splits it into two sub-windows $W_0$ (older) and $W_1$ (newer). At each step, ADWIN tests whether the means of $W_0$ and $W_1$ differ by more than a threshold $\epsilon_{\text{cut}}$:
...
\begin{itemize}
    ...
    \item \textbf{Detection delay:} $O\left(\frac{1}{\alpha \epsilon^2} \ln \frac{1}{\delta}\right)$ observations after a change of magnitude $\epsilon$, where $\alpha \in (0, 1]$ is the regularization parameter governing the fraction of the window affected by the change (smaller $\alpha$ implies gradual drift spread over more of the window, requiring more observations to detect)
    \item \textbf{Memory:} $O(\log W)$ using exponential histogram compression
\end{itemize}

\textbf{On Gradual Drift:} Gradual drift unfolds over extended periods (days to months). ADWIN's sliding window (configured at 1 minute for real-time responsiveness) detects when the \textbf{cumulative effect} of gradual drift produces a statistically significant shift in violation/null rate. The 1-minute window provides rapid responsiveness; drift that develops over weeks is detected as a series of incremental statistical flags, each corresponding to a meaningful change point.
