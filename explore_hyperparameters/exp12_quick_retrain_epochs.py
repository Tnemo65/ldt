...
def run():
    ...
    print("EXP 12: Quick Retrain Epochs")
    print("Loading data...")
    ...
    for n_epochs in grid:
        ...
        results.append({'n_epochs': n_epochs, 'metrics': m})

    best = max(results, key=lambda r: r['metrics']['AUC-ROC'])

    output = {
        'experiment': 'exp12_quick_retrain_epochs',
        'hyperparameter': 'quick_retrain_epochs',
        'timestamp': ts,
        'priority': 'LOW',
        'grid': grid,
        'results': results,
        'best_config': best,
        'recommendation': f"epochs={best['n_epochs']} gives best AUC-ROC.",
    }
    out_path = OUT / f'exp12_quick_retrain_epochs_{ts}.json'
