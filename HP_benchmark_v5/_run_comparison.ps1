...
Write-Host "COMPARISON: MemStream vs baselines, 5000 epochs, 200K train" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
def eval_score(scores, gt):
    ...
    pred=scores>=best_t
    tp,fp,tn,fn=int((pred>gt).sum()),int((pred&~gt).sum()),int((~pred&~gt).sum()),int((~pred&gt).sum())
    prec=tp/(tp+fp+1e-10);rec=tp/(tp+fn+1e-10)
    f1=2*prec*rec/(prec+rec+1e-10)
...
m_if['method'] = 'IsolationForest'; m_if['category'] = 'offline'
m_if['time_s'] = time.time() - t0
print(f'  IF: AUC-PR={m_if[\"auc_pr\"]:.4f} AUC-ROC={m_if[\"auc_roc\"]:.4f} F1={m_if[\"f1\"]:.4f} [{m_if[\"time_s\"]:.1f}s]')
...
# Subsample train to 100K max for offline methods
...
for h_dim in hidden_dims:
    layers.append(nn.Linear(in_d, h_dim)); layers.append(nn.ReLU()); in_d = h_dim
...
for i, h_dim in enumerate(rev_dims):
    ...
    if i+1 < len(rev_dims): decoder_layers.append(nn.ReLU())
...
m_ae['method'] = 'Normal Autoencoder'; m_ae['category'] = 'offline'
m_ae['time_s'] = time.time() - t0
print(f'  AE: AUC-PR={m_ae[\"auc_pr\"]:.4f} AUC-ROC={m_ae[\"auc_roc\"]:.4f} F1={m_ae[\"f1\"]:.4f} [{m_ae[\"time_s\"]:.1f}s]')
...
# ======================================================================
print()
print('[3/4] MemStream v5 (streaming)...')
t0 = time.time()
...
pipe_ms.train(X_train, hours_train, dows_train, rcs_train, nb_train,
...
m_ms['method'] = 'MemStream v5'; m_ms['category'] = 'streaming'
...
print(f'  MS: AUC-PR={m_ms[\"auc_pr\"]:.4f} AUC-ROC={m_ms[\"auc_roc\"]:.4f} F1={m_ms[\"f1\"]:.4f} [{m_ms[\"time_s\"]:.1f}s]')
...
m_oc['method'] = 'One-Class SVM'; m_oc['category'] = 'offline'
m_oc['time_s'] = time.time() - t0
print(f'  OCSVM: AUC-PR={m_oc[\"auc_pr\"]:.4f} AUC-ROC={m_oc[\"auc_roc\"]:.4f} F1={m_oc[\"f1\"]:.4f} [{m_oc[\"time_s\"]:.1f}s]')
...
print(f'{\"Rank\":<6} {\"Method\":<22} {\"Category\":<12} {\"AUC-PR\":>8} {\"AUC-ROC\":>9} {\"F1\":>8} {\"Prec\":>8} {\"Rec\":>8} {\"Time\":>8}')
print('-'*80)
for rank, (key, m) in enumerate(sorted_results, 1):
    print(f'{rank:<6} {m[\"method\"]:<22} {m[\"category\"]:<12} {m[\"auc_pr\"]:>8.4f} {m[\"auc_roc\"]:>9.4f} {m[\"f1\"]:>8.4f} {m[\"precision\"]:>8.4f} {m[\"recall\"]:>8.4f} {m[\"time_s\"]:>7.1f}s')

# ======================================================================
# SAVE RESULTS
# ======================================================================
out_json = {
    'results': {k: {kk: vv for kk, vv in v.items() if not kk.startswith('_')} for k, v in all_results.items()},
