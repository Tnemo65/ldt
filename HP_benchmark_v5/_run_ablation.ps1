Set-Location "c:\proj\ldt\HP_benchmark_v5"

$env:PYTHONPATH = "c:\proj\ldt\HP_benchmark_v5"

Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "ABLATION STUDY: 4 setups, 5000 epochs, 200K train" -ForegroundColor Cyan
Write-Host "============================================================"

& "C:\Users\Administrator\Desktop\AI ComfyUI\system\python\python.exe" -c "
import os
...
# ======================================================================
print()
print('[Setup A] Normal Autoencoder (no noise, no memory)')
t0 = time.time()
...
print(f'  [A] AUC-PR={metrics_a[\"auc_pr\"]:.4f} AUC-ROC={metrics_a[\"auc_roc\"]:.4f} F1={metrics_a[\"f1\"]:.4f} [{elapsed_a:.1f}s]')
all_results['A'] = {'metrics': metrics_a, 'scores': scores_a.tolist(), 'threshold': float(best_t_a), 'elapsed': elapsed_a}

# Save setup A artifacts
...
# ======================================================================
print()
print('[Setup B] Denoise Autoencoder (raw features + noise)')
t0 = time.time()
...
ae_state_b = {
    'input_dim': d7, 'out_dim': out_dim_b,
...
scorer_b = PaperScorer(ae_state=ae_state_b, mean=mean_b, std=std_b,
                        mem_M=memory_b.active(), k=best_cfg.get('k', 5),
...
print(f'  [B] AUC-PR={metrics_b[\"auc_pr\"]:.4f} AUC-ROC={metrics_b[\"auc_roc\"]:.4f} F1={metrics_b[\"f1\"]:.4f} [{elapsed_b:.1f}s]')
all_results['B'] = {'metrics': metrics_b, 'scores': scores_b.tolist(), 'threshold': float(best_t_b), 'elapsed': elapsed_b}
...
# ======================================================================
print()
print('[Setup C] Denoise AE + Feature Engineering (streaming memory)')
t0 = time.time()

pipe_c = MemStreamPipeline(
    d=38, out_dim=best_cfg.get('out_dim', 68),
    memory_len=best_cfg['memory_len'], k=best_cfg['k'],
    gamma=best_cfg['gamma'], beta=best_cfg['beta'],
    noise_std=best_cfg['noise_std'], lr=best_cfg['lr'],
    epochs=EPOCHS, batch_size=1024, seed=SEED,
    cb_warmup=min(4096, best_cfg['memory_len'] * 4),
    verbose=False, adam_betas=(0.9, 0.999)
)
pipe_c.train(X_train_34, hours_train, dows_train, rcs_train, nb_train,
...
adj_scores_c, _ = pipe_c.score_stream(
    X_test_34, hours_test, dows_test, rcs_test, nb_test,
...
print(f'  [C] AUC-PR={metrics_c[\"auc_pr\"]:.4f} AUC-ROC={metrics_c[\"auc_roc\"]:.4f} F1={metrics_c[\"f1\"]:.4f} [{elapsed_c:.1f}s]')
all_results['C'] = {'metrics': metrics_c, 'scores': adj_scores_c.tolist(), 'threshold': float(best_t_c), 'elapsed': elapsed_c, 'epoch_losses': [float(x) for x in epoch_losses_c]}
...
# ======================================================================
print()
print('[Setup D] Denoise AE + FE + ContextBeta (full MemStream)')
t0 = time.time()
...
pipe_d.train(X_train_34, hours_train, dows_train, rcs_train, nb_train,
    X_warmup=X_train_34[:best_cfg['memory_len']],
    hours_warmup=hours_train[:best_cfg['memory_len']],
    dows_warmup=dows_train[:best_cfg['memory_len']],
    rcs_warmup=rcs_train[:best_cfg['memory_len']],
...
Xw_d = X_train_34[:wn_d]; hw_d = hours_train[:wn_d]
dw_d = dows_train[:wn_d]; rcw_d = rcs_train[:wn_d]; nbw_d = nb_train[:wn_d]
...
print(f'  [D] AUC-PR={metrics_d[\"auc_pr\"]:.4f} AUC-ROC={metrics_d[\"auc_roc\"]:.4f} F1={metrics_d[\"f1\"]:.4f} [{elapsed_d:.1f}s]')
all_results['D'] = {'metrics': metrics_d, 'scores': adj_scores_d.tolist(), 'threshold': float(best_t_d), 'elapsed': elapsed_d, 'epoch_losses': [float(x) for x in epoch_losses_d], 'n_mem_updates': ms_d.get('n_memory_updates', 0)}
...
summary_rows = []
for label, desc in [('A','Normal AE'), ('B','Denoise AE'), ('C','AE+FE+Mem'), ('D','AE+FE+CB')]:
    if label in all_results:
        m = all_results[label]['metrics']
        t = all_results[label]['elapsed']
        print(f'{label:<8} {m[\"auc_pr\"]:>8.4f} {m[\"auc_roc\"]:>9.4f} {m[\"f1\"]:>8.4f} {m[\"precision\"]:>8.4f} {m[\"recall\"]:>8.4f} {m[\"separation_ratio\"]:>8.4f} {t:>7.1f}s')
        summary_rows.append({'setup': label, 'desc': desc, **m, 'elapsed': t, 'threshold': all_results[label]['threshold']})
...
# Save JSON summary
output = {
    'epochs': EPOCHS, 'max_train': MAX_TRAIN, 'max_val': MAX_VAL,
    'seed': SEED, 'best_config': best_cfg,
    'summary': summary_rows,
    'deltas': {}
}
if 'A' in all_results and 'B' in all_results:
    output['deltas']['B_vs_A_noise'] = float(all_results['B']['metrics']['auc_pr'] - all_results['A']['metrics']['auc_pr'])
if 'B' in all_results and 'C' in all_results:
    output['deltas']['C_vs_B_FE'] = float(all_results['C']['metrics']['auc_pr'] - all_results['B']['metrics']['auc_pr'])
if 'C' in all_results and 'D' in all_results:
    output['deltas']['D_vs_C_CB'] = float(all_results['D']['metrics']['auc_pr'] - all_results['C']['metrics']['auc_pr'])
if 'A' in all_results and 'D' in all_results:
    output['deltas']['D_vs_A_total'] = float(all_results['D']['metrics']['auc_pr'] - all_results['A']['metrics']['auc_pr'])
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "ABLATION FAILED with exit code $LASTEXITCODE" -ForegroundColor Red
    exit 1
} else {
    Write-Host "ABLATION COMPLETED SUCCESSFULLY" -ForegroundColor Green
}
