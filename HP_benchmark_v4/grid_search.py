...
STAGES:
  Stage 1 (Coarse): 4 * 3 * 3 * 4 = 144 configs, 5000 epochs (paper Table 5/6)
    - memory_len: [256, 1024, 2048, 4096]
    - k: [3, 5, 10]
    - gamma: [0.0, 0.25, 0.5]  # skip 1.0 (Table 6f)
    - beta: [1.0, 0.1, 0.001, 0.0001]  # skip 10.0 (memory freezes)
...
def stage1_coarse(
    ...
    GRID = {
        "memory_len": [512, 1024, 2048],  # 3 values (removed 256, 4096)
        "k": [5, 10],                      # 2 values
        "gamma": [0.0],                    # 1 value (best from paper: gamma=0)
        "beta": [0.001, 0.01],             # 2 values (reduced)
    }
    # Total: 3 * 2 * 1 * 2 = 12 configs
    ...
    for i, cfg in enumerate(configs):
        ...
        try:
            ...
            # Score validation (streaming per-point)
...
def stage3_architecture(
    ...
    arch_grid = {
        "out_dim": [34, 68],      # 2 values (was 5)
        "memory_len": [1024],     # 1 value (best from S2)
        "k": [5, 10],             # 2 values (was 3)
    }
    # Total: 3 * 2 * 1 * 2 = 12 configs
...
def main():
    ...
    parser.add_argument("--epochs-s1", type=int, default=2000,
                        help="Stage 1 epochs (paper: 5000)")
    parser.add_argument("--epochs-s2", type=int, default=2000,
                        help="Stage 2 epochs (paper: 5000)")
    parser.add_argument("--epochs-s3", type=int, default=2000,
    ...
    valid_path = str(v3 / "valid_polluted.parquet")
    ...
    gt_mask = np.load(str(v3 / "valid" / "ground_truth_mask.npy"))
