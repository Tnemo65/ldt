...
for stage in ['stage1', 'stage2']:
    ...
    print(f'{stage}: {len(real_results)} real (>0.25 AUC-PR), {len(corrupted_results)} corrupted')

    for r in corrupted_results[:5]:
        print(f'  CORRUPTED: {r.get("config_id","?")} AUC-PR={r.get("auc_pr",0):.4f}')
    ...
    if real_results:
        print(f'  Best: {real_results[0].get("config_id","?")} AUC-PR={real_results[0].get("auc_pr",0):.4f}')
