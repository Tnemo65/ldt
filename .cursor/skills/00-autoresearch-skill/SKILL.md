...
- **Brainstorm new ideas** using `13-research-ideation/` skills when you're stuck or when results open unexpected questions
...
1. **Search literature** for the research question. Use multiple sources — never stop at one:
   ...
   - **Semantic Scholar** (`pip install semanticscholar`) — best for ML/AI papers, citation graphs, and specific paper lookup. See `12-ml-paper-writing` skill's `references/citation-workflow.md` for complete API code examples
...
3. **Form initial hypotheses** — invoke `13-research-ideation/` skills
...
|---|---|
| Data preparation | `03-data-processing/` |
...
| Optimization (training recipes, mixed precision, scaling) | `08-ml-optimization/` |
| Evaluation / benchmarks | `04-evaluation/` |
| Inference / serving | `12-inference-serving/` |
| Interpretability analysis | `04-mechanistic-interpretability/` |
| Experiment tracking (W&B, MLflow) | `09-mlops/` |
...
6. Generate new hypotheses if warranted (invoke 13-research-ideation/ skills)
...
3. Invoke the `12-ml-paper-writing` skill — it has LaTeX templates for NeurIPS, ICML, ICLR, ACL, AAAI, COLM, and systems venues
...
Don't keep trying random changes. Step back: search literature for related work, invoke `13-research-ideation/` brainstorming skills, or run an outer loop reflection. Being stuck means you need new information or a new perspective, not more experiments.
...
Try multiple approaches in order: Exa MCP for broad search, Semantic Scholar for specific ML/AI paper lookup (`pip install semanticscholar`), arXiv for preprints (`pip install arxiv`). Check `12-ml-paper-writing` skill's `references/citation-workflow.md` for complete API code. Note: Google Scholar has no official API — use Semantic Scholar instead for programmatic search.
