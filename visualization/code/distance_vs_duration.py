"""
Figure 6: Distance vs Duration
===============================
Hexbin scatter plot of trip_distance vs duration_minutes,
colored by VendorID with linear regression fit.
"""
...
def plot():
    ...
    for vid in [1, 2, 6]:
        ...
        sub = df[mask]
        ax.scatter(sub['trip_distance'], sub['duration_min'],
    ...
    if viol_mask.sum() > 0:
        ax.scatter(df.loc[viol_mask, 'trip_distance'],
                   df.loc[viol_mask, 'duration_min'],
                   alpha=0.5, s=12, c=PALETTE[2],
                   label='Violation', marker='x', rasterized=True)

    # Regression line
    valid = df[
        (df['trip_distance'] > 0) & (df['duration_min'] > 0) &
        (df['trip_distance'] < 50) & (df['duration_min'] < 120) &
        (~any_viol)
    ]
    if len(valid) > 1000:
        z = np.polyfit(valid['trip_distance'], valid['duration_min'], 1)
        p = np.poly1d(z)
        x_line = np.linspace(0, 50, 200)
        ax.plot(x_line, p(x_line), color=PALETTE[3], linewidth=2.5,
                linestyle='--', label=f'Linear fit (slope={z[0]:.2f} min/mi)', zorder=5)

    ax.set_xlim(0, 50)
    ax.set_ylim(0, 120)
    ax.set_xlabel('Trip Distance (miles)', fontsize=12)
    ax.set_ylabel('Trip Duration (minutes)', fontsize=12)
    ax.set_title('Distance vs Duration by VendorID (Jan 2024)', fontsize=14, fontweight='bold', pad=12)
