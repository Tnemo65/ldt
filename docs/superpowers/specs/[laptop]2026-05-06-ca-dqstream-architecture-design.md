...
**Document Status**: Simplified Development Specification (Laptop-Friendly)  
**Date**: 2026-05-07  
**Author**: Internship Project - NYC Taxi Data Quality Streaming System  
**Version**: 1.9-Simplified  
**Target**: 8 GB Laptop, 1-5K events/sec (Real NYC Taxi throughput)  
...
### Target Metrics (Simplified Development Mode)

- **Throughput**: 1,000-5,000 events/second (realistic NYC Taxi average, optimized for correctness on laptop)
- **False Positive Rate**: <5% (enforced by context-aware thresholds)
- **Latency**: Sub-second end-to-end processing (PyFlink acceptable, no ONNX optimization needed)
- **Availability**: Zero-downtime model updates and fault tolerance via checkpointing
- **Hardware**: 8 GB RAM laptop (single machine, Docker Compose)
...
**Topology** (Simplified Development Mode):
- **Kafka Broker**: 1 instance (sufficient for 1-5K events/sec)
- **Zookeeper**: 1 node (Kafka cluster coordination)
- **Schema Registry**: 1 instance (Confluent, port 8081)
- **Hardware**: Runs on same laptop as Flink (Docker Compose)

**Topics** (7 total):

| Topic | Partitions | Key | Cleanup Policy | Purpose |
|-------|------------|-----|----------------|---------|
| `taxi-nyc-raw` | 6 | `PULocationID` | delete (7d) | Raw ingestion stream |
| `dq-schema-violations` | 6 | `PULocationID` | delete (7d) | Layer 1 schema rejects |
| `dq-hard-rule-violations` | 6 | `PULocationID` | delete (7d) | Layer 2 Canary rejects |
| `dq-anomaly-scores` | 6 | `PULocationID` | delete (7d) | Layer 2 Complex scores |
| `dq-meta-stream` | 8 | `Neighborhood_ID` | delete (7d) | Layer 3 aggregated metrics |
| `if-model-updates` | 1 | `model_name` | **compact** | Model broadcast (global order) |
| `iec-action-replay` | 4 | `scenario` | delete (7d) | IEC failed actions for retry (V1.8) |

**Rationale**:

- Topics 1-4: Partitioned by `PULocationID` to preserve data locality and causality
- Topic 5: 8 partitions (power-of-2, industry best practice) for `Neighborhood_ID` keying
  - Supports 5-7 neighborhoods currently defined (with headroom for future growth)
  - Power-of-2 ensures even hash distribution across partitions
...
- Topic 7: 4 partitions for action replay (matches Flink parallelism), keyed by `scenario` to group similar failures
...
**Topology** (Simplified Development Mode):
- **JobManager**: 1 instance (2 GB RAM)
- **TaskManager**: 1 instance (4 slots, 8 GB RAM)
- **Total**: 2 containers, 10 GB RAM allocated (fits in 8 GB laptop with swap)
...
# TaskManager (Simplified Development: 1 node, 4 slots)
taskmanager.memory.process.size: 6g  # Reduced for 8 GB laptop
taskmanager.numberOfTaskSlots: 4
taskmanager.network.memory.fraction: 0.3  # CRITICAL: Increased from default 0.1 for MetaAggregator shuffle
...
# RocksDB Memory Management (Prevents OOM)
...
# - Flink 4 slots × 3 connections = 12
...
**Summary Table** (8 GB Laptop):

| Component | Instances | Memory per Instance | Total Memory | CPU Cores | Storage | % of Budget |
|-----------|-----------|---------------------|--------------|-----------|---------|-------------|
| **Flink JobManager** | 1 | 2 GB | 2 GB | 2 | - | 4% |
| **Flink TaskManager** | 1 | 6 GB | 6 GB | 4 | SSD (RocksDB) | 60% |
...
| Metric | Laptop (8 GB) | Production (50 GB) | Ratio |
|--------|-------------|--------------|-------------|
| **Total RAM** | 10 GB (8 GB + 2 GB swap) | 50 GB (no swap) | **5x** |
