...
from _config import RESULTS_DIR, CHARTS_DIR
...
def main():
    ...
    # ---- Grid Search Stage 2 ----
    best_s2, all_s2 = best_of_dir(os.path.join(RESULTS_DIR, "grid_search_s2", "*.json"))
    if best_s2:
    ...
    # ---- Grid Search Stage 3 ----
    best_s3, all_s3 = best_of_dir(os.path.join(RESULTS_DIR, "grid_search_s3", "*.json"))
    if best_s3:
    ...
    # ---- Context Stratified ----
    ctx_path = os.path.join(RESULTS_DIR, "context_stratified", "context_ablation_results.json")
    ...
    # ---- Concept Drift ----
    cd_path = os.path.join(RESULTS_DIR, "concept_drift", "concept_drift_results.json")
    ...
    # ---- Full Data ----
    fd_path = os.path.join(RESULTS_DIR, "full_data", "full_data_results.json")
    ...
    cmp_path = RESULTS_DIR / "comparison" / "comparison_results.json"
    ...
    if all_s2 and len(all_s2) >= 4:
        ...
        plt.tight_layout()
        path = os.path.join(CHARTS_DIR, "stage2_heatmaps.png")
    ...
    if all_s3 and len(all_s3) >= 4:
        ...
        plt.tight_layout()
        path = os.path.join(CHARTS_DIR, "stage3_architecture.png")
        fig.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Chart: {path}")
    ...
    if all_data.get("context"):
        ...
        if segments:
            ...
            plt.tight_layout()
            path = os.path.join(CHARTS_DIR, "contextbeta_ablation.png")
            fig.savefig(path, dpi=150, bbox_inches="tight")
            plt.close()
            print(f"  Chart: {path}")
    ...
    if all_data.get("concept_drift"):
        ...
        plt.tight_layout()
        path = os.path.join(CHARTS_DIR, "concept_drift_comparison.png")
        fig.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Chart: {path}")
    ...
    if all_data.get("full_data"):
        ...
        plt.tight_layout()
        path = os.path.join(CHARTS_DIR, "full_data_training.png")
        fig.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Chart: {path}")
    ...
    if entries:
        ...
        ax.grid(True, alpha=0.3, axis="y")
        plt.tight_layout()
        path = os.path.join(CHARTS_DIR, "overall_summary.png")
        fig.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  Chart: {path}")
