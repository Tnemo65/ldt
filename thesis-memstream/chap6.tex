...
\begin{itemize}
    ...
    \item \textbf{Memory Module:} Stores representative normal patterns from streaming data via a FIFO circular buffer of 8,192 encoded vectors, enabling online adaptation without batch retraining.
...
\textbf{Impact:}
\begin{itemize}
    \item \textbf{F1=0.885 on mixed-fraud scenarios, FPR=2.99\%:} Meets all production targets (FPR $<$ 5\%, Recall $\geq$ 75\% on mixed-fraud).
    \item \textbf{F1=0.877 on hybrid anomalies.}
...
\begin{itemize}
    \item \textbf{Layer 1 -- Baseline Validation:} Schema validation via Confluent Schema Registry (BACKWARD compatibility) and CEL-based business rule contracts, routing invalid records to a dedicated Kafka topic for inspection.
...
\textbf{H2 (Online AE + Memory Module):} Achieves F1 $\geq$ 0.885 on mixed-fraud anomalies and F1 $\geq$ 0.71 weighted average, with FPR $<$ 5\% & \checkmark SUPPORTED & F1=0.885 (mixed), F1=0.877 (hybrid), per-type: 0.087/0.030/0.088, weighted F1=0.71, FPR=2.99\% \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{Memory Capacity:} The FIFO circular buffer (8,192 vectors per neighborhood) may not capture all diverse normal patterns during rapid concept drift, potentially leading to forgetting important patterns.
...
While the Online AE + Memory Module operates online, the memory update and L1 k-NN scoring over 8,192 memory vectors add computational overhead. For very high-throughput scenarios (e.g., $>$100K events/sec), this may become a bottleneck.
...
Replace brute-force k-NN over the 8,192-vector memory buffer with approximate nearest-neighbor indexing (FAISS, HNSW) to support larger memory capacities and higher throughput:
