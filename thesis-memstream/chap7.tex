...
Traditional anomaly detection uses single-feature rules or global ML models. CA-DQStream introduces an \textbf{Online Autoencoder + Memory Module} that combines deep learning reconstruction with streaming memory:

\begin{itemize}
    \item \textbf{Online AE + Memory Module} provides context boundaries (K=10 clusters based on elbow method)
    \item \textbf{K-Means clustering} provides context boundaries for per-cluster anomaly scoring
...
\textbf{Novelty:} First streaming DQ framework to combine clustering-based context boundaries with Online Autoencoder and Memory Module for anomaly detection. Prior hybrid approaches either use clustering for density estimation (not context) or apply ML models globally (not per-context).
...
\begin{itemize}
    ...
    \item \textbf{Quantitative evidence:} Schema Validation catches 100\% of NULL violations; Business Rules catch 99.7\% of validity violations; Online AE + Memory Module catches multivariate anomalies; ADWIN catches consistency drift.
\end{itemize}

\subsection{H2: Online Autoencoder + Memory Module vs. Baselines}
\textbf{H2: Online AE + Memory Module outperforms rule-based and global ML baselines on multivariate anomaly detection.}
\begin{itemize}
    \item \textbf{Supported.} MCC 0.847 vs. 0.612 (Online AE without memory) and 0.489 (rule-based).
    \item \textbf{Supported.} BAR 0.862 vs. 0.641 (Online AE without memory).
...
\subsection{H5: 4D Thresholds vs. Global}
\textbf{H5: 4D context-aware thresholds reduce false positive rate compared to global thresholds.}
\begin{itemize}
